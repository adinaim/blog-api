...
EMAIL_HOST_USER = config('EMAIL_HOST_USER') # почта, с которой будут отправляться письма
EMAIL_PORT = config('EMAIL_PORT', default = 587)
EMAIL_HOST = config('EMAIL_HOST') # какой сервис/хост используется для отправки писем
EMAIL_HOST_PASSWORD = config('EMAIL_HOST_PASSWORD') # пароль от почты
EMAIL_USE_TLS = config('EMAIL_USE_TLS', cast=bool) # config возвращает строку, вид соединения для отправки писем
